...
def main():
    ...
    try:
        ...
        for r in minuta_data:
            ...
            # sesion 27: normalizar mayusculas -- RETAIL a veces viene con casing
            # inconsistente (ALUSWEET: 'Walmart'/'Tottus'/'Cencosud').
            cadena_minuta = str(retail_raw).strip().upper() if retail_raw else None
            # sesion 28: aplicar TAMBIEN el SUFIJO (Tabla6) a MINUTA.COD_LOCAL -- se
            # detecto que ALUSWEET trae el codigo CON prefijo en MINUTA ('EX142', 'HI58',
            # etc.), a diferencia de otros clientes donde ya venia numerico puro. Sin este
            # paso, esas filas quedaban en una clave distinta a las de OSA/REPORTES/VENTAS
            # (que SI se normalizaban), partiendo la misma sala en 2 grupos. normalizar()
            # es segura de aplicar siempre: si el codigo ya viene limpio, lo deja igual.
            cod_local_minuta = normalizar(r[idx4["COD_LOCAL"]], cadena_minuta) if cadena_minuta else norm_code(r[idx4["COD_LOCAL"]])
            minuta_rows.append({
