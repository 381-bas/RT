...
ACCIONES = {
    ...
    "INMOVILIZADO": "Buscar oportunidad de obtener mas caras por venta lenta (producto con 2 semanas sin ventas).",
...
def calcular_riesgo(datos):
    """Riesgo (0 a 1) = 0.5*(tipos/4) + 0.5*(min(focos,6)/6), con OSA e INMOVILIZADO
    ponderados por magnitud de stock (peso_stock), no por presencia binaria."""
    peso_seguimiento = 1.0 if datos["sin_visita"] else 0.0

    pesos_osa = [peso_stock(stock, UMBRAL_STOCK_OSA) for (_prod, stock) in datos["osa_productos"]]
    osa_presente = 1.0 if pesos_osa else 0.0
    osa_peso_total = sum(pesos_osa)

    peso_negativo = 1.0 if datos["negativo"] else 0.0
    negativo_focos = len(datos["negativo"])  # sin ponderar: stock negativo ya es anomalia real

    pesos_inmov = [peso_stock(stock, UMBRAL_STOCK_INMOV) for (_sku, _prod, stock) in datos["inmov"]]
    inmov_severidad = max(pesos_inmov) if pesos_inmov else 0.0
    inmov_peso_total = sum(pesos_inmov)
    ...
    return round(0.5 * (tipos / 4) + 0.5 * (min(focos, 6) / 6), 2)
    ...
    try:
        ...
        for r in minuta_data:
            ...
            if cliente.upper() == "TNOGAL" and es_libro(desc_prod):
                continue
            minuta_rows.append({
                "cadena": r[idx4["RETAIL"]], "cliente": cliente,
        ...
        for (cliente, cadena, cod_local), datos in grupos.items():
            partes = []
            if datos["sin_visita"]:
                partes.append(f"SEGUIMIENTO | {ACCIONES['SEGUIMIENTO']}")

            if cliente in CLIENTES_MENSAJE_GENERAL:
                if datos["osa_productos"] or datos["negativo"] or datos["inmov"]:
                    partes.append(f"REVISION GENERAL | {ACCIONES['REVISION_GENERAL']}")
            else:
                if datos["osa_productos"]:
                    productos_txt = ", ".join(p for p, _s in datos["osa_productos"])
                    partes.append(f"OSA | {productos_txt} | {ACCIONES['OSA']}")
                for sku, prod, stock in datos["negativo"]:
                    partes.append(f"STOCK NEGATIVO | SKU: {fmt_num(sku)} | {prod} | STOCK: {fmt_num(stock)} | {ACCIONES['STOCK NEGATIVO']}")
                if not datos["osa_productos"]:
                    for sku, prod, stock in datos["inmov"]:
                        partes.append(f"INMOVILIZADO | SKU: {fmt_num(sku)} | {prod} | STOCK: {fmt_num(stock)} | {ACCIONES['INMOVILIZADO']}")
