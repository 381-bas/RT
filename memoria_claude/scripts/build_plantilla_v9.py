...
def calcular_riesgo(datos, cliente):
    """Riesgo (0 a 1) = 0.5*(tipos/max_tipos) + 0.5*(min(focos,6)/6), con OSA e
    INMOVILIZADO ponderados por magnitud de stock (peso_stock), y cada tipo ademas
    ...
    return round(0.5 * (tipos / max_tipos) + 0.5 * (min(focos, 6) / 6), 2)
...
def main():
    ...
    try:
        ...
        for (cliente, cadena, cod_local), datos in grupos.items():
            ...
            if not partes:
                continue
            riesgo = calcular_riesgo(datos, cliente)
